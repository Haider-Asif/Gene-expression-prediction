...
def k_cross_validate_model(train_x, train_y, k):
    for i in range(k):
        ...
        layer_1 = tf.keras.layers.Conv1D(1,4,activation=tf.keras.layers.ReLU(), padding="SAME")
        ...
        Dense_1 = tf.keras.layers.Dense(1,activation=tf.keras.layers.ReLU())
        model.add(layer_1)
...
def train_model(train_x, train_y):
    ...
    max_pool_1 = tf.keras.layers.MaxPool1D(3)
    flatten = tf.keras.layers.Flatten()
    Dense_1 = tf.keras.layers.Dense(32,activation=tf.keras.layers.ReLU())
    dropout_1 = tf.keras.layers.Dropout(0.3)
    Dense_2 = tf.keras.layers.Dense(1,activation=None)
