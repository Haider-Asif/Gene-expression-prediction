...
def k_cross_validate_model(train_x, train_y, k):
    ...
    for i in range(k):
        ...
        history = model.fit(x=training_x, y=training_y, batch_size=250, epochs=0, validation_data=(validation_x,validation_y), shuffle=True)
...
def train_model(train_x, train_y):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.01), loss=tf.keras.losses.MeanSquaredError())
    history = model.fit(x=train_x, y=train_y, batch_size=500, epochs=20,shuffle=True)
